...
resource "google_sql_database_instance" "master" {
  ...
  settings {
    ...
    backup_configuration {
      ...
      binary_log_enabled             = var.backup_enabled && locals.database_type == "MYSQL" ? true : false
      location                       = var.backup_location
      start_time                     = var.bcakup_start_time
      point_in_time_recovery_enabled = locals.point_in_time_recovery_enabled
...
resource "google_sql_user" "iam_account" {
  for_each = {
    ...
    "${u.email} ${u.is_service_account}" => iu
  }
